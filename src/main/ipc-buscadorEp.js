    ...
    function registerBuscadorEpIPC() {
        ...
        ipcMain.handle("buscador-ep-download-examples", async () => {
            if (dialogOpen) {
                return { ok: false, error: "Outro diálogo já está aberto." };
            }
            ...
            try {
                const result = await dialog.showOpenDialog({
                title: "Escolha a pasta para salvar as planilhas exemplo",
                ...
                if (!exists) {
                return { ok: false, error: "Pasta de planilhas exemplo não encontrada." };
                }

                const files = await fs.readdir(sourceDir);

                if (!files.length) {
                return { ok: false, error: "Nenhuma planilha exemplo encontrada." };
                }
